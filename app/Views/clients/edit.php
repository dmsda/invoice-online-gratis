        ...
        <div class="card shadow-sm border-0">
            <div class="card-header bg-white py-3 border-bottom">
                <div class="d-flex align-items-center gap-2">
                    <div class="client-avatar bg-warning text-white">
                        <?= strtoupper(mb_substr($client['client_name'], 0, 1)) ?>
                    </div>
                    <div>
                        <h5 class="mb-0 fw-bold">Edit Data Pelanggan</h5>
                        <small class="text-muted"><?= esc($client['client_name']) ?></small>
                    </div>
                </div>
            </div>
            <div class="card-body p-4">
                ...
                <form action="/clients/update/<?= $client['id'] ?>" method="post" x-data="{ loading: false }" @submit="if(loading) { $event.preventDefault(); return; }; loading = true;">
                    <?= csrf_field() ?>
                    ...
                    <div class="mb-4">
                        ...
                        <div class="input-group">
                            ...
                            <textarea name="client_address" class="form-control border-start-0 ps-2" rows="3" 
                                      placeholder="Jl. Contoh No. 123, Kota, Provinsi"><?= esc(old('client_address', $client['client_address'])) ?></textarea>
                        </div>
                    </div>
                    ...
                    <div class="form-actions d-none d-md-block">
                        <div class="d-flex justify-content-between align-items-center">
                            <button type="submit" form="delete-client-form" class="btn btn-outline-danger px-3"
                                    onclick="return confirm('Yakin hapus pelanggan ini? Invoice terkait akan kehilangan referensi nama pelanggan.');">
                                <i class="bi bi-trash3 me-1"></i> Hapus
                            </button>
                            <div class="d-flex gap-2">
                                <a href="/clients/show/<?= $client['id'] ?>" class="btn btn-light px-4">Batal</a>
                                <button type="submit" class="btn btn-primary px-4 rounded-pill" :disabled="loading">
                                    ...
                                    <span x-show="!loading"><i class="bi bi-check-lg me-1"></i> Simpan Perubahan</span>
                    ...
                    <div class="sticky-bottom-bar d-md-none d-flex justify-content-between align-items-center gap-2 p-3 bg-white border-top shadow-lg" style="position: fixed; bottom: 0; left: 0; right: 0; z-index: 1040;">
                        <button type="submit" form="delete-client-form" class="btn btn-outline-danger rounded-pill px-3"
                ...
                <form id="delete-client-form" action="/clients/delete/<?= $client['id'] ?>" method="post">
                    <?= csrf_field() ?>
